...
def run(
    *,
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        (iteration_dir / "prompt.md").parent.mkdir(parents=True, exist_ok=True)
        ...
        try:
            ...
            evidence["iterations"].append({"iteration": iteration, "agent_error": str(exc)})
            return finish("REJECTED", [str(exc)])

        if agent["exit_code"] != 0:
            evidence["iterations"].append({"iteration": iteration, "agent": agent})
        ...
        if scope.passed and all(g["passed"] for g in quick):
            quick_passed = True
            break
