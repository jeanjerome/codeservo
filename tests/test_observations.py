...
import codeservo.observations as observations
from codeservo.evidence import sha256_file
...
PACKAGED_SCHEMA = Path(observations.__file__).with_name("observation.schema.json")
...
class PublishedSchemaTests(unittest.TestCase):
    ...
    def test_the_wheel_carries_the_published_schema(self) -> None:
        pyproject = Path(observations.__file__).resolve().parents[2] / "pyproject.toml"
        ...
        self.assertIn(
            "src/codeservo/observation.schema.json",
